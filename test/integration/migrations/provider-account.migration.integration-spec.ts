...
import { backfillLegacyEstateProviders } from '../../../src/modules/provider/data-migrations/backfill-legacy-estate-providers';

jest.setTimeout(120_000);

type CatalogRow = Record<string, string | null>;

describe('ProviderAccount migration (PostgreSQL integration)', () => {
  ...
  it('creates the ProviderAccount persistence contract', async () => {
    const tableRows: CatalogRow[] = await dataSource.query(`
      SELECT table_name
      FROM information_schema.tables
      WHERE table_schema = current_schema()
        AND table_name = 'tbl_provider_account'
    `);
    ...
    const indexes: CatalogRow[] = await dataSource.query(`
      ...
      WHERE schemaname = current_schema()
        AND tablename = 'tbl_provider_account'
    `);
    const indexByName = new Map(
      indexes.map((index) => [index.indexname, index.indexdef]),
    );
    expect(indexByName.get('uq_provider_account_owner_customer_id')).toContain(
      'UNIQUE',
    );
    expect(indexByName.has('idx_provider_account_status')).toBe(true);
    expect(indexByName.has('idx_provider_account_type')).toBe(true);

    const foreignKeys: CatalogRow[] = await dataSource.query(`
      SELECT conname, pg_get_constraintdef(oid) AS definition
      FROM pg_constraint
      WHERE conrelid = 'tbl_provider_account'::regclass
        AND contype = 'f'
    `);
    const ownerForeignKey = foreignKeys.find(
      (foreignKey) =>
        foreignKey.conname === 'fk_provider_account_owner_customer',
    );
    ...
    expect(checksByName.get('chk_provider_account_type')).toEqual(
    ...
    expect(checksByName.get('chk_provider_account_status')).toEqual(
    ...
    expect(
      checksByName.get('chk_provider_account_verification_status'),
    ).toEqual(expect.stringContaining('UNVERIFIED'));
    expect(
      checksByName.get('chk_provider_account_verification_status'),
    ).toEqual(expect.stringContaining('PENDING'));
    expect(
      checksByName.get('chk_provider_account_verification_status'),
    ).toEqual(expect.stringContaining('VERIFIED'));
    expect(
      checksByName.get('chk_provider_account_verification_status'),
  ...
  it('backfills legacy Estate providers idempotently', async () => {
    ...
    const userRows: CatalogRow[] = await dataSource.query(
      `
        INSERT INTO tbl_customer_account (email, password, role_id)
        VALUES ('backfill-provider@nexus.test', 'not-used', $1)
        RETURNING id
      `,
      [roleRows[0].id],
    );
    ...
    const firstRun = await backfillLegacyEstateProviders(dataSource);
    const secondRun = await backfillLegacyEstateProviders(dataSource);
    const accountRows: CatalogRow[] = await dataSource.query(
      `SELECT id FROM tbl_provider_account WHERE owner_customer_id = $1`,
      [userRows[0].id],
    );

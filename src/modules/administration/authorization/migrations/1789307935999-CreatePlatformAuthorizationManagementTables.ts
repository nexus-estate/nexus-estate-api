...
export class CreatePlatformAuthorizationManagementTables1789307935999 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE tbl_marketplace_role (
    ...
    `);
    await queryRunner.query(`
      CREATE TABLE tbl_marketplace_permission (
        id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    `);
    await queryRunner.query(`
      CREATE TABLE tbl_marketplace_role_permission (
    ...
    `);
    await queryRunner.query(`
      CREATE INDEX idx_marketplace_role_permission_permission_id
        ON tbl_marketplace_role_permission(permission_id)
    `);
    await queryRunner.query(`
      CREATE TABLE tbl_customer_role_assignment (
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX idx_customer_role_assignment_role_id ON tbl_customer_role_assignment(role_id)`,
    );

    await queryRunner.query(`
      CREATE TABLE tbl_provider_membership (
        id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    `);
    await queryRunner.query(`
      CREATE INDEX idx_provider_membership_provider_id
        ON tbl_provider_membership(provider_id)
    `);
    await queryRunner.query(`
      CREATE INDEX idx_provider_membership_customer_id
        ON tbl_provider_membership(customer_id)
    `);
    await queryRunner.query(`
      CREATE TABLE tbl_provider_role (
        id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
        code varchar(100) NOT NULL,
        name varchar(255) NOT NULL,
        description text NULL,
    ...
    `);
    await queryRunner.query(`
      CREATE TABLE tbl_provider_permission (
        id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
        code varchar(150) NOT NULL,
        name varchar(255) NOT NULL,
        description text NOT NULL,
    ...
    `);
    await queryRunner.query(`
      CREATE TABLE tbl_provider_role_permission (
        role_id uuid NOT NULL,
        permission_id uuid NOT NULL,
        PRIMARY KEY (role_id, permission_id),
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX idx_provider_role_permission_permission_id ON tbl_provider_role_permission(permission_id)`,
    );
    await queryRunner.query(`
      CREATE TABLE tbl_provider_membership_role (
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX idx_provider_membership_role_role_id ON tbl_provider_membership_role(role_id)`,
    );

    await queryRunner.query(`
      CREATE TABLE tbl_authorization_audit_log (
        id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    `);
    await queryRunner.query(`
      CREATE INDEX idx_authorization_audit_platform_created_at
    ...
    await queryRunner.query(`
      ...
      ON CONFLICT (code) DO UPDATE
        SET name = EXCLUDED.name, is_system = true, status = 'ACTIVE', deleted_at = NULL
    `);
    await queryRunner.query(`

import { BackfillAdministratorRoleAssignments1789307935998 } from './1789307935998-BackfillAdministratorRoleAssignments';

describe('BackfillAdministratorRoleAssignments', () => {
  it('uses an idempotent scoped SUPER_ADMIN backfill', async () => {
    const query = jest.fn().mockResolvedValue(undefined);
    await new BackfillAdministratorRoleAssignments1789307935998().up({
      query,
    } as never);
  ...
  it('removes only SUPER_ADMIN assignments on down', async () => {
    const query = jest.fn().mockResolvedValue(undefined);
    await new BackfillAdministratorRoleAssignments1789307935998().down({
      query,
    } as never);
    const calls = query.mock.calls as unknown as Array<[string]>;

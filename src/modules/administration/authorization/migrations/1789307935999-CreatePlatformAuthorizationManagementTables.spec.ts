import { CreatePlatformAuthorizationManagementTables1789307935999 } from './1789307935999-CreatePlatformAuthorizationManagementTables';
...
describe('CreatePlatformAuthorizationManagementTables', () => {
  it('expands all platform tables and includes owner backfill without destructive drops', async () => {
    ...
    await new CreatePlatformAuthorizationManagementTables1789307935999().up({
      query,
    } as never);
  ...
  it('provides an explicit rollback for the expand migration', async () => {
    const query = jest.fn().mockResolvedValue(undefined);
    await new CreatePlatformAuthorizationManagementTables1789307935999().down({
      query,
    } as never);
    expect(

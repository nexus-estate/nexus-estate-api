...
type RepositoryMock = {
  ...
  findByOwnerCustomerId: jest.MockedFunction<ProviderAccountRepository['findByOwnerCustomerId']>;
  existsByOwnerCustomerId: jest.MockedFunction<ProviderAccountRepository['existsByOwnerCustomerId']>;
...
describe('ProviderAccountService', () => {
  ...
  it('creates a pending account with server-controlled defaults', async () => {
    ...
    await expect(service.createForCustomer(customerId, dto)).resolves.toMatchObject({
      id: providerId,
      displayName: 'Nguyen Van A',
      status: ProviderStatus.ACTIVE,
      verificationStatus: ProviderVerificationStatus.PENDING,
  ...
  it.each(['', '   '])('rejects blank display name %p', async (displayName) => {
    await expect(
      ...
      errorCode: ProviderAccountErrorCodes.PROVIDER_ACCOUNT_INVALID_DISPLAY_NAME.code,
    });
  });
  ...
  it('gets the current account without auto-creating it', async () => {
    ...
    await expect(service.getCurrent(customerId)).resolves.toMatchObject({ id: providerId });
  ...
  it('updates only the display name', async () => {
    repository.findByOwnerCustomerId.mockResolvedValue(account);
    repository.findById.mockResolvedValue(account);
    repository.update.mockResolvedValue({ ...account, displayName: 'Updated Name' });
  ...
  it('blocks supply while provider approval is pending', async () => {
    repository.findByOwnerCustomerId.mockResolvedValue(account);
    await expect(service.requireActiveProvider(customerId)).rejects.toMatchObject({
